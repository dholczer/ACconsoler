    ...
    public static class StringToObject
    {
        ...
        private static object ConvertSingleValue(string value, Type argumentType)
        {
            ...
            if (argumentType.IsPrimitive || argumentType == typeof(decimal) || argumentType.IsEnum)
            {
                ...
                {
                    throw new AConsolerException("Could not convert \"{0}\" to {1}", value, argumentType.ToString());
                }
                catch (OverflowException)
                {
                    throw new AConsolerException("Value \"{0}\" is too big or too small", value);
                }
            }

            throw new AConsolerException("Unknown type is used in your method: {0}", argumentType.FullName);
        }
        ...
        private static DateTime ConvertToDateTime(string parameter)
        {
            ...
            if (parts.Length != 3)
            {
                throw new AConsolerException("Could not convert {0} to Date", parameter);
            }
